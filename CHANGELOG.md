...
### Added
- `preValidated` option on `runs.save()`, `runs.validate()`, `runs.update()`, and `runs.updateById()` — pass `{ preValidated: true }` as the second (or third) argument to bypass client-side Zod validation. Designed for MCP and other pre-validated callers that already validate input before reaching the SDK, avoiding redundant double validation.
